...
exports.verificaUtilizador = async (req, res) => {
    ...
    const resposta = { message: "The user is active!"};
    ...
    try {
      ...
      db.Crud_registar(email, password, confirmationToken) // C: Create
        .then((dados) => {
          ...
          res.status(201).send({
            message:
              "User created successfully, check your mailbox to activate!",
          });
    ...
    db.cRud_login(email) //
      .then(async (dados) => {
        if (await bcrypt.compare(req.body.password, dados.password)) {
          ...
          return res.status(401).send({ erro: "Password is not correct!" });
        }
      })
      .catch((response) => {
